using AucklandRide.UWP.Controls;
using AucklandRide.UWP.Models;
using AucklandRide.UWP.Views;
using System;
...
namespace AucklandRide.UWP
{
    ...
    public sealed partial class AppShell : Page
    {
        ...
        public AppShell()
        {
            ...
            var listViewItem = navlist[0];
            NavMenuList.SelectedItem = listViewItem;
            AppFrame.Navigate(listViewItem.DestPage, listViewItem.Arguments, new Windows.UI.Xaml.Media.Animation.SuppressNavigationTransitionInfo());
        }

        public Frame AppFrame { get { return this.frame; } }

        public void Navigate(string label, object arguments)
        {
            var listItem = NavMenuList.Items.Cast<NavMenuItem>().FirstOrDefault(x => x.Label == label);
            listItem.Arguments = arguments;
            if (listItem != null)
            {
                if (listItem.DestPage != null &&
                    listItem.DestPage != this.AppFrame.CurrentSourcePageType)
                {
                    this.AppFrame.Navigate(listItem.DestPage, listItem.Arguments);
                }
                NavMenuList.SelectedItem = listItem;
            }
        }
        ...
        private void OnNavigatingToPage(object sender, NavigatingCancelEventArgs e)
        {
            if (e.NavigationMode == NavigationMode.Back)
            {
                ...
                AppFrame.Navigate(item.DestPage, item.Arguments, new Windows.UI.Xaml.Media.Animation.SuppressNavigationTransitionInfo());
